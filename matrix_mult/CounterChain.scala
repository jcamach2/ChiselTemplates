...
class CounterChain(n : Int, w : Int) extends Module {
	...
	val counters = for (i <- 0 until n) yield
					{
						val one_counter = Module(new ItrCounter(w))
						one_counter
					}
	...
	for (i <- 0 until n) {
		...
		io.counters_cout(i) := counters(i).io.cout
	}

	counters(0).io.en_count := io.en 

	for (j <- 1 until n) {
		/* once a counter reaches max, done will remain 1 (true) until reset is hit */
		counters(j).io.en_count := counters.dropRight(n - j) map (_.io.done) reduceLeft(_ && _)
		/* AND all previous done signals */
	}

	io.done_complete := counters(n - 1).io.en_count && counters(n - 1).io.done
}
